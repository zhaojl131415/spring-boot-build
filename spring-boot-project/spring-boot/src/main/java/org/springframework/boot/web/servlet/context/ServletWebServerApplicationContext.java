...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	/**
	 * spring容器初始化refresh()方法
	 * org.springframework.context.support.AbstractApplicationContext#refresh()
	 * 中调用了onRefresh();
	 * 这里重写了onRefresh()方法, 在这一步创建了web服务器
	 */
	@Override
	protected void onRefresh() {
		...
		try {
			/**
			 * 创建web服务器, 启动
			 * 在这里判断是外部web服务器还是内嵌web服务器, 如果是内嵌在这里创建了web服务器
			 */
			createWebServer();
		}
		catch (Throwable ex) {
	...
	private void createWebServer() {
		...
		if (webServer == null && servletContext == null) {
			/**
			 * Web服务器工厂类: 有多个实现类: tomcat,jetty,undertow
			 * 获取Web服务器工厂类: 获取web服务器的具体实现.
			 */
			ServletWebServerFactory factory = getWebServerFactory();
			/**
			 * @see JettyServletWebServerFactory#getWebServer(org.springframework.boot.web.servlet.ServletContextInitializer...)
			 * @see TomcatServletWebServerFactory#getWebServer(org.springframework.boot.web.servlet.ServletContextInitializer...)
			 * @see UndertowServletWebServerFactory#getWebServer(org.springframework.boot.web.servlet.ServletContextInitializer...)
			 */
	...
	protected ServletWebServerFactory getWebServerFactory() {
		...
		/**
		 * 从spring容器中, 根据类型获取对应的beanName数组
		 * 在spring初始化的时候, 通过spi指定了实现.
		 * 自动配置实现
		...
		// 根据beanName获取对应的web服务器的具体实现: spring容器中获取
